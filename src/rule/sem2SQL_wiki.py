...
from semQL import Sup, Sel, Order, Root, Filter, A, N, C, T, Root1
...
if __name__ == '__main__':
    ...
    with open(args.output_path, 'w', encoding='utf8') as d, open('gold.txt', 'w', encoding='utf8') as g:
        for i in index:
            ...
            # replace corrsponding gold SQL by new table columns
            #result = transform(datas[i], schemas[datas[i]['db_id']], label = True)
            #g.write("%s\t%s\t%s\n" % (result[0], datas[i]["db_id"], datas[i]["question"]))
            g.write("%s\t%s\t%s\n" % (datas[i]['sql'], datas[i]["db_id"], datas[i]["question"]))
            #g.write("%s\t%s\n" % (datas[i]['query'], datas[i]["db_id"]))
    
    data_sql = pd.read_csv('gold.txt', sep = '\t', names = ['query', 'db_id', 'question'])
    data_sql.sort_values(by = 'db_id', inplace = True)
    data_sql.to_csv('./gold.sql', sep = '\t', index = None, header = None)
