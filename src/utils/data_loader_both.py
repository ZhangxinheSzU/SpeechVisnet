...
from src.utils.dataset import Example, Batch
...
def process(sql, sentence, table):
    ...
    process_dict['sentence'] = sentence
...
def get_col_table_dict(tab_cols, tab_ids, sql):
    ...
    # 这个col_table_dict是说col和哪几个table有关系？
...
def to_example(sql, sentence, table, tts_filename,ifeval = False):
    process_dict = process(sql, sentence, table)
    ...
    if 'rule_label' in sql:
        try:
            ...
            return None
        if utils_semQL.is_valid(rule_label, col_table_dict = col_table_dict, sql=sql) is False:
            return None
    names_unique_list = []
    for item in sql['names']:
        if item not in names_unique_list:
            names_unique_list.append(item)
    names_unique_list[0] = 'all'
    schema = sql['table_names']+names_unique_list
    # schema = sql['names'][1:]
    table = sql['table_names']
    schema_seqs = [' '.join([wordnet_lemmatizer.lemmatize(v.lower()) for v in x.split()]) for x in schema]
    table_seqs = [' '.join([wordnet_lemmatizer.lemmatize(v.lower()) for v in x.split()]) for x in table]
    example = Example(
            nl = sql['nl_queries'][sql['id2']],
            src_sentence = process_dict['sentence'],
...
class SpectrogramParser(object):
    def __init__(self, normalize=False, augment=False):
        """
...
class SpeechSQLDataset(Dataset, SpectrogramParser):
    def __init__(self,type, sql_path, speech_path, table_path, normalize=False, augment=False, ifeval = False, if_parse_audio = True, mode = 'train',processor=None):
        '''
        ...
        self.train_speech_path = cfg.train_wav_path
        self.test_speech_path = cfg.test_wav_path
        #if len(self.sql_data) > 9000:
        #    self.sql_data = self.sql_data[9000:19000]
        self.ifeval = ifeval
        self.if_parse_audio = if_parse_audio
        if ifeval:
            self.sql_data = self.sql_data#[2000:]     
        # self.resampler = transforms.Resample(22050,16000)
        # if cfg.cuda: 
        #         self.resampler.cuda()
    ...
    def __getitem__(self, index):
        ...
        # if 'dev' in tts_filename:
        #     tts_filename = tts_filename[4:]
        ...
        #     wav_path = self.dev_speech_path.format(tts_filename)
        # elif self.mode=='test':
        #     wav_path = self.test_speech_path.format(tts_filename)
        # else:
        #     wav_path = self.train_speech_path.format(tts_filename)
        if self.mode !='test500':
            if self.mode=='new_train' or self.mode=='new_dev'or self.mode=='new_test':
                tts_filename = sql_data['id']
                wav_path = '/home/zhoujh/speech2SQL/data/new/wav/{}.wav'.format(tts_filename)
            elif self.mode=='clone_train' or self.mode=='clone_dev'or self.mode=='clone_test':
                tts_filename = sql_data['id']
                wav_path = '/home/zhoujh/speech2SQL/data/voiceClone/{}.wav'.format(tts_filename)
            elif self.mode=='train' or self.mode=='dev'or self.mode=='test':
                ...
                if self.mode=='dev':
                    ...
                    wav_path = self.train_speech_path.format(tts_filename)
            elif self.mode=='small_train' or self.mode=='small_dev'or self.mode=='small_test':
                tts_filename = sql_data['filename']
                # tts = sql_data['tts']
                # if tts=='OldTest':
                #     wav_path = self.train_speech_path.format(tts_filename)
            ...
            tts_filename = sql_data['wav']
            wav_path ='/home/zhoujh/speech2SQL/data/nvbench/test500/{}.wav'.format(sql_data['wav'])
        
        # if self.type=='speech'or self.type=='speech_seperate' or self.type=='speech_one_schema'or self.type=='speech_one'or self.type=='speech_schema'or self.type=='speech_schemaAttention'or self.type=='speech_schema_straight':
        if self.if_parse_audio:
            # spect = self.parse_audio(wav_path, self.ifeval)
            spect=wav_path
            # spect = self.resampler(spect)
            # spect = self.processor(spect, sampling_rate=self.target_sample_rate, return_tensors="pt").input_values
            # spect = spect.squeeze(dim=0)
            # # spect = spect.squeeze(dim=0)
            sentence = spect
        else:
            ...
            sentence = spect
        # else:
        #     sentence = sql_data['nl_queries'][sql_data['matchnl']]
        #     sentence = remove(sentence.lower())
        #     sentence = [[wordnet_lemmatizer.lemmatize(x)] for x in sentence.split()]
        #print(spect.shape)

        if sentence is None:
            return None
        
        example = to_example(sql_data, sentence, table_data,tts_filename, self.ifeval)
        return example
...
def _collate_fn(batch):
    ...
    # descending sorted
    batch = sorted(batch, key=lambda sample: len(sample.src_sentence), reverse=True)
    return batch
