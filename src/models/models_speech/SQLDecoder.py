...
class SQLDecoder(nn.Module):
    def __init__(self, grammar):
        ...
        # decoder cell
        self.lf_decoder_lstm = nn.LSTMCell(input_dim, args.DECODER.hidden_size) # for detail
        self.sketch_decoder_lstm = nn.LSTMCell(input_dim, args.DECODER.hidden_size) # for sketch
        ...
        self.schema_rnn_input = nn.Linear(args.DECODER.hidden_size, args.DECODER.action_embed_size, bias=False)
        # 我不是很清楚这部分有什么意义
        self.schema_embedding_linear = nn.Linear(args.DECODER.hidden_size, args.DECODER.hidden_size, bias=False)  # TODO: if need for attention?
        self.schema_pointer_net = pointer_net.PointerNet(args.DECODER.hidden_size, args.DECODER.col_embed_size, attention_type=args.DECODER.column_att)
    ...
    def forward(self, batch, encoder_outputs, schema_outputs):
        ...
        schema_embedding = self.schema_embedding_linear(schema_embedding)
        ...
        for t in range(batch.max_action_num):
                    ...
                    if t < len(example.tgt_actions):
                        ...
                        if type(action_tm1) in [define_rule.Root0,
                            ...
                            if isinstance(action_tm1, define_rule.C):
                                ...
                                a_tm1_embed = self.schema_rnn_input(schema_embedding[e_id, action_tm1.id_c + table_num])
                            elif isinstance(action_tm1, define_rule.T):
                                a_tm1_embed = self.schema_rnn_input(schema_embedding[e_id, action_tm1.id_c])
            ...
            weights = self.schema_pointer_net(src_encodings = schema_embedding, query_vec = att_t.unsqueeze(0),
            ...
            schema_attention_weights = F.softmax(weights, dim=-1)
            # now get the loss
            for e_id, example in enumerate(examples):
                table_num = example.table_len
                if t < len(example.tgt_actions):
                    ...
                    if isinstance(action_t, define_rule.C):
                        ...
                        act_prob_t_i = schema_attention_weights[e_id, action_t.id_c + table_num]
                        action_probs[e_id].append(act_prob_t_i)
                        #print('C', act_prob_t_i)
                        
                    elif isinstance(action_t, define_rule.T):
                        act_prob_t_i = schema_attention_weights[e_id, action_t.id_c]
                        action_probs[e_id].append(act_prob_t_i)
    ...
    def parse(self, batch, encoder_outputs, schema_outputs, beam_size = 5, match_outputs = None):
        ...
        while len(completed_beams) < beam_size and t < cfg.DECODER.decode_max_time_step:
            ...
            if t == 0:
                ...
                for e_id, hyp in enumerate(beams):
                    ...
                    if type(action_tm1) in [define_rule.Root0,
                        ...
                        a_tm1_embed = self.schema_rnn_input(schema_embedding[0, action_tm1.id_c + table_num])
                    elif isinstance(action_tm1, define_rule.T):
                        a_tm1_embed = self.schema_rnn_input(schema_embedding[0, action_tm1.id_c])
                    elif isinstance(action_tm1, define_rule.A):
                        a_tm1_embed = self.production_embed.weight[self.grammar.prod2id[action_tm1.production]]
                    else:
                        raise ValueError('unknown action %s' % action_tm1)
            ...
            weights = self.schema_pointer_net(src_encodings=exp_schema_embedding, query_vec=att_t.unsqueeze(0),
                                                  src_token_mask=batch.schema_token_mask)
            ...
            for hyp_id, hyp in enumerate(beams):
                ...
                if type(padding_sketch[t]) == define_rule.A:
                    ...
                    for col_id, _ in enumerate(batch.table_sents[0]):
                        ...
                        try:
                            # 这个感觉是选择col最关键的一步
                            col_sel_score = column_selection_log_prob[hyp_id, col_id + table_num] 
                        except:
                    ...
                    for t_id, _ in enumerate(batch.table_names[0]):
                        t_sel_score = column_selection_log_prob[hyp_id, t_id]
